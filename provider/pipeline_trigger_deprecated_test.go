...
func TestAccPipelineTriggerDeprecated(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccPipelineTriggerConfigDeprecated(pipeName, newJenkinsMaster, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
			...
			{
				Config: testAccPipelineTriggerConfigDeprecated(pipeName, jenkinsMaster, 1),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(trigger1, "master", jenkinsMaster),
					resource.TestCheckResourceAttr(trigger1, "property_file", "build.properties.1"),
					testAccCheckPipelineExists(pipelineResourceName, &pipelineRef),
			...
			{
				Config: testAccPipelineTriggerConfigDeprecated(pipeName, jenkinsMaster, 0),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckPipelineExists(pipelineResourceName, &pipelineRef),
...
func testAccPipelineTriggerConfigDeprecated(pipeName string, master string, count int) string {
	triggers := ""
	for i := 1; i <= count; i++ {
