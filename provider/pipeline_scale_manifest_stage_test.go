...
func init() {
	stageTypes["spinnaker_pipeline_scale_manifest_stage"] = client.ScaleManifestStageType
}

func TestAccPipelineScaleManifestStageBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPipelineScaleManifestStageConfigBasic(pipeName, accountName, 2),
			...
			{
				Config: testAccPipelineScaleManifestStageConfigBasic(pipeName, newAccountName, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(stage1, "name", "Stage 1"),
			...
			{
				Config: testAccPipelineScaleManifestStageConfigBasic(pipeName, accountName, 0),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckPipelineExists(pipelineResourceName, &pipelineRef),
...
func testAccPipelineScaleManifestStageConfigBasic(pipeName string, account string, count int) string {
	stages := ""
	for i := 1; i <= count; i++ {
		stages += fmt.Sprintf(`
resource "spinnaker_pipeline_scale_manifest_stage" "s%v" {
