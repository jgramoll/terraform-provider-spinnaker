...
func init() {
	stageTypes["spinnaker_pipeline_disable_manifest_stage"] = client.DisableManifestStageType
}

func TestAccPipelineDisableManifestStageBasic(t *testing.T) {
	...
	stage1 := "spinnaker_pipeline_disable_manifest_stage.s1"
	stage2 := "spinnaker_pipeline_disable_manifest_stage.s2"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPipelineDisableManifestStageConfigBasic(pipeName, accountName, 2),
			...
			{
				Config: testAccPipelineDisableManifestStageConfigBasic(pipeName, newAccountName, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(stage1, "name", "Stage 1"),
			...
			{
				Config: testAccPipelineDisableManifestStageConfigBasic(pipeName, accountName, 1),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(stage1, "name", "Stage 1"),
					resource.TestCheckResourceAttr(stage1, "account", accountName),
					testAccCheckPipelineExists(pipelineResourceName, &pipelineRef),
			...
			{
				Config: testAccPipelineDisableManifestStageConfigBasic(pipeName, accountName, 0),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckPipelineExists(pipelineResourceName, &pipelineRef),
...
func testAccPipelineDisableManifestStageConfigBasic(pipeName string, accountName string, count int) string {
	stages := ""
	for i := 1; i <= count; i++ {
		stages += fmt.Sprintf(`
resource "spinnaker_pipeline_disable_manifest_stage" "s%v" {
