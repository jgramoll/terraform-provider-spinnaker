...
func (s *deleteManifestStage) toClientStage(config *client.Config) (client.Stage, error) {
	...
	if s.Options != nil && len(*s.Options) > 0 {
		cs.Options = (*s.Options)[0].toClientOptions()
	}
...
func (s *deleteManifestStage) fromClientStage(cs client.Stage) stage {
	...
	if clientStage.Options != nil {
		*newStage.Options = append(*newStage.Options, fromClientDeleteManifestOptions(clientStage.Options))
	}
