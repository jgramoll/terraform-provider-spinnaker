...
func TestAccPipelineRollbackClusterStageBasic(t *testing.T) {
	...
	stage1 := "spinnaker_pipeline_rollback_cluster_stage.stage-1"
	stage2 := "spinnaker_pipeline_rollback_cluster_stage.stage-2"
...
func testAccPipelineRollbackClusterStageConfigBasic(pipeName string, targetHealthyRollbackPercentage string, count int) string {
	...
	for i := 1; i <= count; i++ {
		stages += fmt.Sprintf(`
resource "spinnaker_pipeline_rollback_cluster_stage" "stage-%v" {
